...
const ChildView = Backbone.View.extend({
  ...
  initialize: function initialize(options) {
    ...
    this.loadImages();
    this.cloneSectionNav();

    
    if(!(sessionStorage.isDiff)){
      sessionStorage.setItem('isDiff', 1)
    }
    this.sectionNum = window.location.pathname
    if(this.sectionNum.indexOf("diff") < 0){
      sessionStorage.isDiff = 1
    }
  },
  ...
  cloneSectionNav: function cloneSectionNav(){
    var sectionNav = document.getElementById("child_nav_section");
    if(sectionNav != null){
      var clonedNav = sectionNav.cloneNode(true);
      $(clonedNav).addClass("cloned-nav")
      $(clonedNav).find(".next-prev").css("border-top", "none")
      $(clonedNav).find(".next-prev-title").remove()
      $("#regulation_content").prepend(clonedNav);
    }
  },
